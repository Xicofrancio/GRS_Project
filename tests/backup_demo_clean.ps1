...
try {
    # Use System.Net.Http.HttpClient for proper file upload
    ...
    $byteArrayContent.Headers.ContentDisposition = 'form-data; name="file"; filename="demo_file.txt"'
    $byteArrayContent.Headers.ContentType = [System.Net.Http.Headers.MediaTypeHeaderValue]::Parse("text/plain")
    
    $multipartContent.Add($byteArrayContent)
    
    $response = $httpClient.PostAsync("http://localhost:3000/upload", $multipartContent).Result
...
try {
    ...
    Write-Host "SUCCESS: Files in storage:" -ForegroundColor Green
...
Write-Host "Step 4: Viewing file content..." -ForegroundColor Yellow
...
try {
    ...
    Write-Host "ERROR: Failed to view file: $($_.Exception.Message)" -ForegroundColor Red
}

# Step 5: Delete the file (simulate data loss)
Write-Host "Step 5: Deleting file (simulating data loss)..." -ForegroundColor Yellow

try {
    $deleteResponse = Invoke-RestMethod -Uri "http://localhost:3000/api/files/demo_file.txt" -Method Delete
    Write-Host "SUCCESS: File deleted successfully!" -ForegroundColor Green
...
try {
    ...
    Write-Host "SUCCESS: Files after deletion:" -ForegroundColor Green
    ...
    Write-Host "ERROR: Failed to verify deletion: $($_.Exception.Message)" -ForegroundColor Red
}
...
try {
    ...
    Write-Host "SUCCESS: Backup triggered successfully!" -ForegroundColor Green
...
try {
    ...
    Write-Host "SUCCESS: Files restored successfully!" -ForegroundColor Green
...
try {
    ...
    Write-Host "SUCCESS: Files after restore:" -ForegroundColor Green
    $listAfterRestore.files | ForEach-Object { 
        Write-Host "  - $($_.name) (Size: $($_.size) bytes)" -ForegroundColor Cyan 
    }
} catch {
    Write-Host "ERROR: Failed to verify restore: $($_.Exception.Message)" -ForegroundColor Red
}

# Step 10: View restored file content
Write-Host "Step 10: Viewing restored file content..." -ForegroundColor Yellow
...
try {
    ...
    if ($restoreListResponse.files.Count -gt 0) {
        ...
        Write-Host "SUCCESS: Restored file content:" -ForegroundColor Green
        Write-Host "$viewRestoredResponse" -ForegroundColor Cyan
    }
} catch {
    Write-Host "ERROR: Failed to view restored file: $($_.Exception.Message)" -ForegroundColor Red
}
...
Write-Host "Bacula console available at: http://localhost:9095" -ForegroundColor Cyan
